...
class KEGG:
    def __init__(self, pathway_id=None, KGML_file=None):
        ...
        if pathway_id is not None:
            ...
            self.root = ET.fromstring(
                requests.get("http://rest.kegg.jp/get/{}/kgml".format(pathway_id)).text
            )
        ...
        # Create graph
        self.G_kegg = nx.from_pandas_edgelist(
            self.edge_attributes_df,
            "source",
            "target",
            edge_attr=True,
    ...
    def _get_entry_attributes_as_dataframe(self):
        ...
        entry_type_df = pd.DataFrame([entry.attrib for entry in self._entries]).drop(
            columns=["name", "link"], errors="ignore"
        )
        entry_graphics_df = pd.DataFrame(
            [entry.find("graphics").attrib for entry in self._entries]
        ).rename(columns={"name": "aliases", "type": "shape"})
        ...
        entry_attributes_df["name"] = entry_attributes_df["aliases"].apply(
        ...
        entry_attributes_df[["x", "y", "height", "width"]] = entry_attributes_df[
            ["x", "y", "height", "width"]
        ].astype(float)
    ...
    def _get_edge_attributes_as_dataframe(self):
        ...
        for relation_attributes in relation_attributes_list:
            ...
            existing_edges = [
                set([edge_attributes["source"], edge_attributes["target"]])
                for edge_attributes in edge_attributes_list
            ]

            # Add edge attribute if the edge has not been seen before
            if (
                set([relation_attributes["source"], relation_attributes["target"]])
        ...
        edge_attributes_df = pd.DataFrame(
            edge_attributes_list, columns=self.edge_columns
        )
    ...
    def _get_directed_edge_attributes_as_dataframe(self, edge_attributes_df):
        ...
        if len(edge_attributes_df) == 0:
            return edge_attributes_df

        reverse_edges_df = edge_attributes_df[
            edge_attributes_df["effect"].isin([-2, 0, 2])
        ].rename(columns={"source": "target", "target": "source"})
        directed_edge_attributes_df = pd.concat(
            [edge_attributes_df, reverse_edges_df], sort=False
        )

        return directed_edge_attributes_df

    def _infer_gene_edges_from_reactions(self):
        compound_ids = self.node_attributes_df[
            self.node_attributes_df["type"] == "compound"
        ].index

        directed_edge_attributes_df = self._get_directed_edge_attributes_as_dataframe(
            self.edge_attributes_df
        )

        inferred_edges = []
        oriented_edge_attributes_df = directed_edge_attributes_df[
            directed_edge_attributes_df["effect"] != 0
        ]

        for compound_id in compound_ids:
            sourced_compounds_df = oriented_edge_attributes_df[
                oriented_edge_attributes_df["source"] == compound_id
            ]
            targeted_compounds_df = oriented_edge_attributes_df[
                oriented_edge_attributes_df["target"] == compound_id
            ]
            ...
            for source, target in product(source_nodes, target_nodes):
                inferred_edges.append(
                    self._populate_edge_attributes(
                        source, target, "inferred_rxn", ["activation"]
                    )
                )

        inferred_edges_df = pd.DataFrame(
            inferred_edges, columns=self.edge_columns
        ).drop_duplicates()

        # Remove duplicated edges, consolidate bidirectional edges
        edgelist_as_sets = [
            set(pair) for pair in inferred_edges_df[["source", "target"]].values
        ]
        inferred_edges_df["effect"] = [
            edgelist_as_sets.count(pair) for pair in edgelist_as_sets
        ]
        inferred_edges_df = inferred_edges_df[
            [
                False if pair in edgelist_as_sets[:i] else True
                for i, pair in enumerate(edgelist_as_sets)
            ]
        ]
    ...
    def _populate_edge_attributes(self, source, target, edge_type, interactions):
        ...
        for interaction in interactions:
            if interaction == "binding/association":
                edge_attributes.update({"effect": 2})
            elif interaction == "protein complex":
                edge_attributes.update(
                    {"effect": 2}
                )  # not standard type, but including for clarity
            elif interaction == "bidirected":
                edge_attributes.update(
                    {"effect": 2}
                )  # not standard type, but including for clarity
            elif interaction == "dissociation":
                edge_attributes.update({"effect": 1})
            elif interaction == "missing interaction":
                edge_attributes.update({"effect": 0})
            elif interaction == "indirect effect":
                edge_attributes.update({"effect": 1, "indirect": 1})
            else:
                pass

        for interaction in interactions:
            if interaction == "phosphorylation":
                edge_attributes.update({"effect": 1, "modification": "+p"})
            elif interaction == "dephosphorylation":
                edge_attributes.update({"effect": 1, "modification": "-p"})
            elif interaction == "glycosylation":
                edge_attributes.update({"effect": 1, "modification": "+g"})
            elif interaction == "ubiquitination":
                edge_attributes.update({"effect": 1, "modification": "+u"})
            elif interaction == "methylation":
                edge_attributes.update({"effect": 1, "modification": "+m"})

        for interaction in interactions:
            if interaction == "activation":
                edge_attributes.update({"effect": 1})
            elif interaction == "inhibition":
                edge_attributes.update({"effect": -1})
            elif interaction == "expression":
                edge_attributes.update({"effect": 1, "modification": "e"})
    ...
    def _get_edge_attributes_from_reactions(self):
        ...
        for reaction in self._reactions:
            compound_id, reaction_name, reaction_type = (
                reaction.get("id"),
                reaction.get("name"),
                reaction.get("type"),
            )
            substrate_ids = [
                substrate.get("id") for substrate in reaction.findall("substrate")
            ]
            ...
            for substrate_id in substrate_ids:
                if reaction_type == "irreversible":
                    reaction_attributes_list.append(
                        self._populate_edge_attributes(
                            substrate_id, compound_id, reaction_name, ["activation"]
                        )
                    )
                else:
                    reaction_attributes_list.append(
                        self._populate_edge_attributes(
                            compound_id, substrate_id, reaction_name, ["activation"]
                        )
                    )
            ...
            for product_id in product_ids:
                reaction_attributes_list.append(
                    self._populate_edge_attributes(
                        compound_id, product_id, reaction_name, ["activation"]
                    )
                )
    ...
    def _get_edge_attributes_from_relations(self):
        ...
        for relation in self._relations:
            ...
            edge_descriptors = [
                subtype.get("name") for subtype in relation.findall("subtype")
            ]

            # TODO: add support for maplinks?
            if edge_type in ["ECrel", "PPrel", "GErel", "PCrel"]:
                if "compound" not in edge_descriptors:
                    relation_attributes_list.append(
                        self._populate_edge_attributes(
                            source, target, edge_type, edge_descriptors
                        )
                    )

                else:
                    # Get compound id by first searching through subtypes with `name` attribute equal to 'compound', then retrieving 'value'
                    compound_id = relation.find('.//subtype[@name="compound"]').get(
                        "value"
                    )

                    relation_attributes_list.append(
                        self._populate_edge_attributes(
                            source, compound_id, edge_type, edge_descriptors
                        )
                    )
                    relation_attributes_list.append(
                        self._populate_edge_attributes(
                            compound_id, target, edge_type, edge_descriptors
                        )
                    )
    ...
    def _replace_group_edges(self, edge_attributes_df):
        for group_element in self._groups:
            ...
            group_members = [
                component.get("id") for component in group_element.findall("component")
            ]
            ...
            for node_type in ["source", "target"]:
                edges_with_df = edge_attributes_df[
                    edge_attributes_df[node_type] == group_id
                ]
                edges_without_df = edge_attributes_df[
                    edge_attributes_df[node_type] != group_id
                ]
                # Duplicate rows where `node1` contains the `group_id`
                expanded_edges_df = pd.concat(
                    [edges_with_df] * len(group_members)
                ).sort_index()
                ...
                edge_attributes_df = pd.concat(
                    [expanded_edges_df, edges_without_df]
                ).reset_index(drop=True)

            # Add edges *between* group members. Complexed proteins are essentially `binding/association`
            group_rows = [
                self._populate_edge_attributes(a, b, "PComplex", ["protein complex"])
                for a, b in combinations(group_members, 2)
            ]
            edge_attributes_df = pd.concat(
                [
                    edge_attributes_df,
                    pd.DataFrame(group_rows, columns=self.edge_columns),
                ],
                ignore_index=True,
            ).fillna(0)
    ...
    def output_KGML_as_directed_networkx(self, genes_only=True):
        ...
        if genes_only:
            inferred_edge_attributes_df = (
                self._get_directed_edge_attributes_as_dataframe(
                    self.inferred_edge_attributes_df
                )
            )
            directed_edge_attributes_df = pd.concat(
                [directed_edge_attributes_df, inferred_edge_attributes_df]
            )

            graph = nx.from_pandas_edgelist(
                directed_edge_attributes_df,
                "source",
                "target",
                edge_attr=True,
                create_using=nx.DiGraph(),
            ...
            graph = nx.DiGraph(
                graph.subgraph(
                    self.node_attributes_df.index[
                        self.node_attributes_df["type"] == "gene"
                    ]
                )
            )
        ...
        nx.relabel_nodes(
            graph,
            {node_id: graph.nodes[node_id]["name"] for node_id in graph.nodes()},
            copy=False,
        )
    ...
    def output_KGML_as_graphml(self, path, visualize="full"):
        ...
        if len(self.edge_attributes_df) > 0:
            graph = nx.from_pandas_edgelist(
                self.edge_attributes_df.fillna(""),
                "source",
                "target",
                edge_attr=True,
                create_using=nx.DiGraph(),
        ...
        if visualize == "full":
            ...
            inferred_edges_graph = nx.from_pandas_edgelist(
                self._infer_gene_edges_from_reactions(),
                "source",
                "target",
                edge_attr=True,
                create_using=nx.DiGraph(),
    ...
    def view(self, scale=1, show_compounds=False, gene_values=None):
        entry_attributes_df = self.entry_attributes_df.replace("", np.nan).dropna(
            subset=["name"]
        )

        fig, ax = set_grid(
            xlim=entry_attributes_df.x.agg(["min", "max"]).tolist(),
            ylim=entry_attributes_df.y.agg(["min", "max"]).tolist(),
            scale=scale,
        )
        ...
        if gene_values is not None:
            ...
            hex_lookup = [
                color for color in sns.color_palette("coolwarm", 256).as_hex()
            ] + ["#b3b3b3"]
            ...
            rgb_indices = (
                ((gene_values / gene_values.abs().max() + 1) / 2 * 255)
                .fillna(-1)
                .astype(int)
            )
            ...
            if node.shape == "circle":
                ax.add_patch(
                    matplotlib.patches.Circle(
                        xy=node.center, radius=node.width / 2, color="k", fill=False
                    )
                )
                if show_compounds:
                    ax.text(
                ...
                ax.text(
                    x=node.center[0],
                    y=node.center[1] + 1,
                    s=node.name[6:] if node.name.startswith("TITLE:") else node.name,
                    fontsize=6 * scale,
                    ha="center",
                    va="center",
                    wrap=True,
        ...
        for _, edge_attribs in self.edge_attributes_df.iterrows():
            ...
            source_pos, target_pos = shortest_arrow(
                nodes_dic[edge_attribs["source"]], nodes_dic[edge_attribs["target"]]
            )

            arrowprops = dict(color="k")
            if edge_attribs["indirect"] == 1:
                arrowprops["linestyle"] = "--"
            elif edge_attribs["indirect"] == 0:
                arrowprops["linestyle"] = "-"
            else:
                arrowprops["linestyle"] = "-"
...
def output_DiGraph_as_graphml(graph, path):
    ...
    for source, target in list(graph_out.edges):
        if graph_out.has_edge(target, source):
            graph_out.remove_edge(source, target)
